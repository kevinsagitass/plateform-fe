...
import { X, AlertTriangle, Plus, Pencil, Layers, Hash } from "lucide-react";
import { AddonGroup, CreateAddonGroupPayload } from "@/types/organizationMenu";
import ModalWrapper from "./ModalWrapper";

interface AddonGroupModalProps {
  mode: "add" | "edit";
  organizationMenuId?: string | null;
  group?: AddonGroup;
  onClose: () => void;
  onSubmit: (payload: CreateAddonGroupPayload) => Promise<void>;
}
...
const AddonGroupModal = ({
  ...
  const [maxSelection, setMaxSelection] = useState(group?.maxSelection ?? 1);
  ...
  const validate = () => {
    ...
    if (maxSelection < 1) e.maxSelection = "Must allow at least 1 selection.";
    return e;
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-neutral-900 dark:text-neutral-100 font-display">
              {isEdit ? "Edit Addon Group" : "Add Addon Group"}
            </h2>
            <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">
              {isEdit
                ? "Update group settings"
                : "Create a new addon group for this item"}
            </p>
          </div>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
            Group Name <span className="text-accent-500">*</span>
          </label>
          <div className="relative">
            <Layers className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 dark:text-neutral-500" />
            <input
              type="text"
              value={name}
              onChange={(e) => {
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
            Max Selections <span className="text-accent-500">*</span>
          </label>
          <div className="relative">
            <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 dark:text-neutral-500" />
            <input
              type="number"
              min={0}
              value={maxSelection}
              onChange={(e) => {
                setMaxSelection(Number(e.target.value));
                if (errors.maxSelection)
                  setErrors((p) => ({ ...p, maxSelection: "" }));
              }}
              className={`${inputBase} pl-9 ${
                errors.maxSelection ? inputError : inputNormal
              }`}
            />
          </div>
          <FieldError field="maxSelection" />
          <p className="text-xs text-neutral-400 dark:text-neutral-500">
            How many addons a customer can pick from this group (0 means no
            maximum)
          </p>
        </div>
